...
fun verifyParentTabs(device:UiDevice,context:android.content.Context) {
    ...
    tap("设置")
    for((entry,label) in listOf("偏好与记忆" to "添加明确偏好","使用摘要" to "待审核偏好：","孩子与成长" to "基准年龄（3—17）","声音" to "中文声音","隐私与权限" to "允许会话内看图")) {
        tap(entry)
        assertTrue("家长二级页：$entry",device.wait(Until.hasObject(By.textContains(label)),10000))
        device.pressBack();Thread.sleep(400)
    }
    tap("首页");tap("播放清单");find("创建播放清单");tap("英语计划");find("添加计划")
    device.pressBack();Thread.sleep(400);device.pressBack();Thread.sleep(400);tap("使用安排");find("每日使用分钟，0表示不设额度")
